defmodule NotKnittingWeb.UserAuth do
  use NotKnittingWeb, :verified_routes
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_not_knitting_web_user_remember_me"
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      NotKnittingWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_user:
      ...
      defmodule NotKnittingWeb.PageLive do
        use NotKnittingWeb, :live_view

        on_mount {NotKnittingWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{NotKnittingWeb.UserAuth, :ensure_authenticated}] do
  ...
  def on_mount(:require_user_owns_pattern, %{"id" => pattern_id}, _session, socket) do
    pattern = Patterns.get_pattern!(pattern_id)
    if socket.assigns.current_user.id == pattern.user_id do
      {:cont, socket}
    else
      socket =
        socket
